...
fn the_view_mount_lists_the_union_and_serves_admitted_bytes() {
    ...
    // Bytes do not come through the view: a write is refused. The namespace
    // is another matter since D169 (`unlink`) and D170 (`rename`, `mkdir`,
    // `rmdir`) — an arr upgrades, renames and tidies; their tests are
    // `d169_view_unlink.rs` and `d170_view_rename.rs`.
