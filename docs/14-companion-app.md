...
- **Loopback HTTP(S)** (`127.0.0.1`, stable port 7421 by default, per-launch token in a well-known `0600` file): the "Sign in with PVFS" surface for browsers/apps. Never bound to a non-loopback address. Serves **https** with a self-generated `localhost` cert kept next to the vault (PVOS M3.6 §4a; offered to the macOS login keychain once) — the port is dual-mode (TLS ClientHello sniffed per connection), so plain-http callers keep working through the transition.
...
**The PVFS surface is built** (phases 1–7): the phase-7 additions — the `ApprovalContext` field on `sign`/tenant-sign, the `pvfsd` challenge consumer, and the explicit `api_version` handshake — landed per doc 16 §7. The `pvos.sso` service that consumes it is PVOS-side (their doc 07 §3.5).
...
5. ☑ **Approval UI + §4 controls** — a `Prompter` seam behind the policy's `Decision::Prompt`: **desktop dialog** where a GUI session exists (`osascript` on macOS, `zenity` on Linux), **terminal prompt** on `/dev/tty` otherwise (serialized, default-deny on EOF/error), and headless stays deny — `--allow-root` remains the explicit server opt-in. `serve --prompt <auto|deny|terminal|desktop>` overrides the auto-detection: scripts and services pass `deny` so a prompt can never block them (auto-detection sees the *script's* tty, not the human). **Rate limit**: sliding-window signatures/minute (default 60, `--rate-limit`, 0 = off). **Audit log**: append-only `0600` JSONL next to the vault (`companion.audit.jsonl`) of every signing decision (approved/denied/rate_limited/locked/error, with type + origin + digest) plus serve_start/lock/unlock events; best-effort by design. **Lock**: `pvfs-companion lock` (new `Lock` protocol op) and an idle timeout (default 15 min, `--idle-lock-secs`) drop the seed from memory; a locked agent **re-unlocks on demand** through an `Unlocker` that retains no secrets (keychain refetch / env re-read / fresh terminal prompt), so aggressive locking costs nothing. 5 agent unit tests (prompter-approved root sign vs headless deny, rate limit + audit trail, lock without unlocker, lock + re-unlock same key, idle lock + restore). ☐ Deferred: the optional Touch ID / biometric unlock gate (macOS LocalAuthentication).
6. ☑ **Identity agent** — a dependency-free loopback HTTP/1.1 listener (`127.0.0.1:0`, `connection: close`) with the §6 security model layered: loopback binding, a **per-launch token** published in a `0600` port file next to the socket (`pvfs-companion.http`), a required browser-enforced `Origin` header, and **wallet-style connect gating** — `POST /connect` prompts through the phase 5 `Prompter` (`approve_connect`, headless = deny), grants persist as `0600` JSON next to the vault with a 30-day TTL, re-read on every check so `pvfs-companion origins revoke <origin>` bites a running agent immediately. `POST /sign-in {challenge}` signs a 32-byte challenge with the **identity key** for a connected origin (no per-request prompt — the connect is the approval; rate limit + lock still apply) and `GET /identity` returns the pubkey; the web path exposes **identity assertions only**, so a root event is unreachable by construction (§4). CORS reflects the caller's origin (the token + connect are the boundary). Audited: `connect` approved/denied per origin, and every web signature under its real origin. CLI: `pvfs-companion origins [revoke]`; `status` shows the endpoint + connected-origin count. Integration tests: token gate, connect → sign-in with signature verified against the identity key, origin isolation, bad-challenge shapes, headless denial, live revocation. ☑ The real `pvfsd` challenge consumer is wired (phase 7): `tests/signin_pvfsd.rs` closes the doc 16 §6 loop against a live daemon.
7. ☑ **Joint API + spec** — the §7 superset surface is pinned in [doc 16](16-joint-agent-api.md) and the PVFS side is built: `ApprovalContext` on `sign`/tenant-sign (rendered by the `Prompter`, recorded in the audit log, digest-consistency-checked), the `user_action` request type (identity key, prompt-by-default), the `api_version` op on both sockets, and the `pvfsd` challenge-consumer test. The `pvos.sso` broker is PVOS-side work against that contract.
...
- **Identity ⇄ device-key relationship for the owner's *own* writes.** In the owner's own forest both
  ...
  **DECIDED (2026-08-13, doc 18 §4 + addendum):** the forest device key signs local auto-routed
  admin; the box's **client identity** authenticates every outbound dial; `device.key` never dials.
