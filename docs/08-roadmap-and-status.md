...
| **Companion** | Root/identity key vault + local signer + localhost identity agent ("Sign in with PVFS") | ◑ **in progress** ([doc 14](14-companion-app.md)): vault ✅, signer + policy ✅, Unix-socket agent ✅, `pvfs-companion` binary + `--via-companion` admit ✅; OS keychain / approval UI / web agent ☐ (desktop work) |
