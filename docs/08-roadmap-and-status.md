...
| **P4** | Federation: `@server` ≠ local, remote catalog, sync; **torrent-like swarm**; **sub-forest (tree/region) replication & sharing** (PVOS-driven: per-app backup, peer-hosting, isolated-app cross-host links) | ◑ **in progress** — phased in [doc 17](17-federation-and-sync.md); **F0 `pvfs export`** (native tree view), **F1 network transport** (`pvfsd --listen`, pinned TLS, single-use nonces), **F2 replicas** (`pvfs replica add/sync` — verified log shipping, read-only serving) built; F3 placement/sync next |
