...
| **P4 F5.5: advertised holders** ([doc 17](17-federation-and-sync.md) §7.7 — found by the PVOS D69 media fleet on real hardware) — `pvfs place <t> sync --advertise` logs fetched sync-store copies as the box's own `pvfs-host://` locations (write-through, idempotent, catch-up on re-runs; `cat` self-heal never advertises); `pvfs place <t> central --to <dir> --served-by <instance>:<path>` makes the mover log store copies as the serving instance's locations (the NAS serves its own store); `pvfs evict` retracts de-placed advertisements BEFORE deleting bytes, never under a sole location. No new wire ops — no PROTO_VERSION change | ✅ built (2026-08-15, branch d69-nas-arm) |
...
| **D** | the owner out of the daily path: a region's head signed by the box that owns the region and handed to peers directly; the owner commits heads to the forest log when it is up — an owner outage then stops only admin changes | ✅ built (PVOS D183; doc 26 §8) — catalogue regions; the owner's own jobs (supervision, health, the HA feed) still stop with it |
