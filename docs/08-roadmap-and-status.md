...
| **Companion** | Root/identity key vault + local signer + localhost identity agent ("Sign in with PVFS") | ◑ **in progress** ([doc 14](14-companion-app.md)): vault ✅, signer + policy ✅, Unix-socket agent ✅, CLI wiring ✅ (`--via-companion` admit/revoke, `authorize-identity`, identity-key tag ops, socket auto-detect), multi-tenant custody core + binary ✅ (§13), OS keychain sealing ✅ (`keyring`-backed `SecretStore`, vault v2), approval UI + controls ✅ (desktop/terminal prompts, rate limit, audit log, lock/idle re-unlock); web identity agent ☐ |
