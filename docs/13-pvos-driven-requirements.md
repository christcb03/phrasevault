...
- *(Superseded 2026-09-23 by PVOS D182's availability track — doc 08: a single forest gets its availability from standby promotion (built), region heads off the owner's path (D183), and later a one-button (B) or quorum-fired (C, opt-in) takeover; "active-active" becomes two forests that cross-link (E), each with its own writer, the same mechanism as sharing between users. The paragraph below is kept as the June reasoning.)*
